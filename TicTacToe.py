...
class TicTacToe:
    def __init__(self):
        ...
        for i in range(3):
            row = []
            for j in range(3):
                button = tk.Button(self.window,text='',width=10,height=5,command=lambda i=i,j=j:self.make_move(i,j))
    ...
    def make_move(self,row,col):
        if self.board[row][col] == '':
            ...
            if self.check_winner(self.current_player):
                messagebox.showinfo('Gameover', 'the winner is '+self.current_player)
                self.window.quit()
            elif self.is_draw():
